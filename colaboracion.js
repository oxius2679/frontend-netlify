...
(function() {
    ...
    // ============================================
    const Core = {
        // ✅ SINCRONIZAR PROYECTOS EXISTENTES (AUTOMÁTICO)
        syncExistingProjects: function() {
            ...
            console.log('🔄 Sincronizando proyectos existentes...');
            let sincronizados = 0;
            ...
            if (sincronizados > 0) {
                saveData();
                console.log(`✅ ${sincronizados} proyecto(s) sincronizados`);
            }
    ...
    function togglePanel() {
        ...
        // Un usuario es dueño si su clienteId coincide con el del proyecto
        ...
        console.log('   - ¿Es dueño de proyecto?', esDuenoDeProyecto);
        ...
        // ✅ SECCIÓN DE INVITAR (solo si puede)
    ...
    // ============================================
    function init() {
        console.log('🚀 Inicializando sistema de colaboración (VERSIÓN DEFINITIVA)...');
        loadData();
        
        // ✅ PASO 1: Sincronizar proyectos existentes (AUTOMÁTICO)
        Core.syncExistingProjects();
        
        // ✅ PASO 2: Sincronizar al sistema principal
        setTimeout(function() {
    ...
    let ultimoProjects = localStorage.getItem('projects') || '';
    
    setInterval(function() {
        const nuevoSync = localStorage.getItem('_sync_timestamp') || '0';
        const nuevosProjects = localStorage.getItem('projects') || '';
        
        if (nuevoSync !== ultimoSync || nuevosProjects !== ultimoProjects) {
            ultimoSync = nuevoSync;
            ultimoProjects = nuevosProjects;
            
            if (nuevosProjects) {
                try {
                    const nuevosProyectos = JSON.parse(nuevosProjects);
                    const cambios = JSON.stringify(projects) !== JSON.stringify(nuevosProyectos);
                    if (cambios) {
                        projects.length = 0;
                        projects.push(...nuevosProyectos);
                        ...
                        if (typeof updateStatistics === 'function') updateStatistics();
                        Core.syncExistingProjects();
                        showSyncNotification('🔄 Datos sincronizados');
                    }
                } catch(e) {}
            }
        }
