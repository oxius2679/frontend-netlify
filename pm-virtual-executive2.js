...
window.renderGantt = function(container) {
  ...
  const proyecto = window.obtenerProyectoActual ? window.obtenerProyectoActual() : null;
  if (!proyecto) {
    container.innerHTML = '<div style="text-align:center; padding:60px; color:#94a3b8;">' + window.translations.ganttNoProject + '</div>';
    return;
  }

  const tasks = proyecto.tasks || [];
  if (tasks.length === 0) {
    container.innerHTML = '<div style="text-align:center; padding:60px; color:#94a3b8;">' + window.translations.ganttNoTasks + '</div>';
    return;
  }
  ...
  const riesgoNivel = rezagadas > 3 ? window.translations.ganttRiskExtreme : (rezagadas > 0 ? window.translations.ganttRiskCritical : (criticas > 2 ? window.translations.ganttRiskHigh : window.translations.ganttRiskMedium));
  const riesgoColor = riesgoNivel === window.translations.ganttRiskExtreme ? '#dc2626' : (riesgoNivel === window.translations.ganttRiskCritical ? '#f97316' : (riesgoNivel === window.translations.ganttRiskHigh ? '#f59e0b' : '#10b981'));
  ...
  const chartLabelsStatus = [window.translations.ganttCompleted, window.translations.ganttInProgress, window.translations.ganttPending, window.translations.ganttOverdue];
  ...
  for (let i = 0; i < tasksSorted.length; i++) {
    ...
    let priorityBadge = '';
    if (t.priority === 'high') priorityBadge = '<span style="background:#dc2626; color:white; padding:2px 8px; border-radius:20px; font-size:9px;">🔴 ' + window.translations.ganttPriorityHigh + '</span>';
    else if (t.priority === 'medium') priorityBadge = '<span style="background:#f59e0b; color:white; padding:2px 8px; border-radius:20px; font-size:9px;">🟡 ' + window.translations.ganttPriorityMedium + '</span>';
    else priorityBadge = '<span style="background:#10b981; color:white; padding:2px 8px; border-radius:20px; font-size:9px;">🟢 ' + window.translations.ganttPriorityLow + '</span>';

    const statusText = t.status === 'completed' ? window.translations.ganttStatusCompleted : (t.status === 'inProgress' ? window.translations.ganttStatusInProgress : window.translations.ganttStatusPending);
    const isOverdue = t.deadline && new Date(t.deadline) < hoy && t.status !== 'completed';
    const overdueBadge = isOverdue ? '<span style="background:#ef4444; color:white; padding:2px 6px; border-radius:20px; font-size:8px; margin-left:6px;">' + window.translations.ganttStatusOverdue + '</span>' : '';

    ganttRowsHtml += '<div class="gantt-row" data-task="' + escapeHtml(t.name) + '" data-start="' + (t.startDate || 'N/A') + '" data-end="' + (t.deadline || 'N/A') + '" data-assignee="' + (t.assignee || window.translations.ganttAssignedTo + '?') + '" data-status="' + t.status + '">';
    ...
    ganttRowsHtml += '<div style="font-size: 10px; color: #94a3b8; margin-top: 4px;">' + (t.assignee || window.translations.ganttAssignedTo + '?') + ' • ' + statusText + '</div>';
    ganttRowsHtml += '</div>';
  ...
  for (let i = 0; i < tasksSorted.slice(0, 15).length; i++) {
    ...
    tasksTableHtml += '<td style="padding: 12px;"><span style="background: ' + statusColor + '20; color: ' + statusColor + '; padding: 4px 12px; border-radius: 20px; font-size: 11px;">' + (t.status === 'completed' ? window.translations.ganttStatusCompleted : (t.status === 'inProgress' ? window.translations.ganttStatusInProgress : window.translations.ganttStatusPending)) + '</span>' + (isOverdue ? ' <span style="background:#ef4444; color:white; padding:2px 8px; border-radius:12px; font-size:9px;">' + window.translations.ganttStatusOverdue + '</span>' : '') + '</td>';
    tasksTableHtml += '<td style="padding: 12px;">' + (t.priority === 'high' ? '🔴 ' + window.translations.ganttPriorityHigh : (t.priority === 'medium' ? '🟡 ' + window.translations.ganttPriorityMedium : '🟢 ' + window.translations.ganttPriorityLow)) + '</td>';
  ...
  if (riesgoNivel === window.translations.ganttRiskExtreme || riesgoNivel === window.translations.ganttRiskCritical) {
    ...
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .today-line-container::before {
        content: '📅 ${window.translations.ganttLegendToday}';
        position: absolute;
        top: -28px;
        left: -28px;
        background: #f59e0b;
  ...
  <body>
    <div class="report-container">
      <div class="btn-group no-print">
        <button class="btn-pdf" id="btnExportPDF">${window.translations.ganttExportPDF}</button>
        <button class="btn-print" id="btnPrint">${window.translations.ganttPrint}</button>
      </div>
          ...
          <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap;">
            <div>
              <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px;">
                ...
                <div>
                  <h1 style="margin: 0; font-size: 32px; font-weight: 700; color: white;">${window.translations.ganttTitle}</h1>
                  <p style="margin: 8px 0 0 0; color: #94a3b8;">${window.translations.ganttSubtitle}</p>
                </div>
              </div>
              <div style="display: flex; gap: 16px; flex-wrap: wrap;">
                ...
                <div style="background: rgba(239,68,68,0.2); backdrop-filter: blur(10px); padding: 6px 20px; border-radius: 40px;">
                  <span style="color: #f87171;">⚠️ ${window.translations.ganttRiskLevel}: ${riesgoNivel}</span>
                </div>
              </div>
            </div>
            <div style="background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); border-radius: 28px; padding: 20px 32px; text-align: center;">
              <div style="font-size: 11px; color: #94a3b8; letter-spacing: 1px;">${window.translations.ganttProgress}</div>
      ...
      <div class="kpi-grid">
        <div class="kpi-card"><div class="kpi-value">${totalTareas}</div><div class="kpi-label">${window.translations.ganttTotalTasks}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#10b981;">${completadas}</div><div class="kpi-label">✅ ${window.translations.ganttCompleted}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#3b82f6;">${enProgreso}</div><div class="kpi-label">🔄 ${window.translations.ganttInProgress}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#f59e0b;">${pendientes}</div><div class="kpi-label">⏳ ${window.translations.ganttPending}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#ef4444;">${rezagadas}</div><div class="kpi-label">🔴 ${window.translations.ganttOverdue}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#f97316;">${criticas}</div><div class="kpi-label">⚠️ ${window.translations.ganttCritical}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#f59e0b;">${hitos.length}</div><div class="kpi-label">⭐ ${window.translations.ganttMilestones}</div></div>
        <div class="kpi-card"><div class="kpi-value" style="color:#8b5cf6;">${spi}</div><div class="kpi-label">📈 ${window.translations.ganttSPI}</div></div>
      </div>
      ...
      <div class="two-columns">
        ...
        <div class="card">
          ...
          <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div><div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><span>${window.translations.ganttProgress} vs Plan</span><span style="font-weight:700;">${porcentajeAvance}%</span></div><div style="background:#e2e8f0; height: 8px; border-radius: 10px; overflow:hidden;"><div style="width: ${porcentajeAvance}%; height:100%; background: #3b82f6; border-radius:10px;"></div></div></div>
            <div><div style="display: flex; justify-content: space-between; margin-bottom: 8px;"><span>Priority Distribution</span></div><div style="display: flex; gap: 8px; margin-top: 8px;"><div style="flex:${prioridadAlta}; background:#dc2626; height: 8px; border-radius: 4px;"></div><div style="flex:${prioridadMedia}; background:#f59e0b; height: 8px; border-radius: 4px;"></div><div style="flex:${prioridadBaja}; background:#10b981; height: 8px; border-radius: 4px;"></div></div><div style="display: flex; justify-content: space-between; margin-top: 8px;"><span style="font-size:10px;">🔴 ${window.translations.ganttPriorityHigh}: ${prioridadAlta}</span><span style="font-size:10px;">🟡 ${window.translations.ganttPriorityMedium}: ${prioridadMedia}</span><span style="font-size:10px;">🟢 ${window.translations.ganttPriorityLow}: ${prioridadBaja}</span></div></div>
      ...
      <div class="gantt-container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
          <div><h3 style="margin:0;">📈 ${window.translations.ganttTimeline}</h3><p style="margin:4px 0 0; font-size:12px; color:#64748b;">Detailed schedule with critical path</p></div>
          <div class="no-print">
            <select id="filterStatusGantt" style="padding:8px 16px; border-radius:40px; border:1px solid #e2e8f0; background:white;">
              <option value="all">${window.translations.ganttFilterAll}</option>
              <option value="completed">${window.translations.ganttFilterCompleted}</option>
              <option value="inProgress">${window.translations.ganttFilterInProgress}</option>
              <option value="pending">${window.translations.ganttFilterPending}</option>
            </select>
            <button id="resetFiltersGantt" style="padding:8px 16px; border-radius:40px; border:1px solid #e2e8f0; background:white; margin-left:8px; cursor:pointer;">${window.translations.ganttClearFilters}</button>
          </div>
        </div>
        <div class="gantt-wrapper" id="ganttWrapper">
          <div class="gantt-header">
            <div class="gantt-task-header">${window.translations.ganttTask}</div>
            <div class="gantt-timeline-header" id="timelineHeader">${timelineDatesHtml}</div>
            <div style="width:80px; text-align:right;">${window.translations.ganttEndDate}</div>
          </div>
        ...
        <div class="legend">
          <div class="legend-item"><div class="legend-color" style="background:#10b981;"></div><span>${window.translations.ganttLegendCompleted}</span></div>
          <div class="legend-item"><div class="legend-color" style="background:#3b82f6;"></div><span>${window.translations.ganttLegendInProgress}</span></div>
          <div class="legend-item"><div class="legend-color" style="background:#f59e0b;"></div><span>${window.translations.ganttLegendPending}</span></div>
          <div class="legend-item"><div class="legend-color" style="background:#ef4444;"></div><span>${window.translations.ganttLegendOverdue}</span></div>
          <div class="legend-item"><div class="legend-circle"></div><span>${window.translations.ganttMilestones}</span></div>
          <div class="legend-item"><div style="width:2px; height:16px; background:#f59e0b;"></div><span>${window.translations.ganttLegendToday}</span></div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom: 32px;">
        ...
        <div style="overflow-x: auto;">
          <table class="tasks-table">
            <thead>
              <tr><th>${window.translations.ganttTask}</th><th>${window.translations.ganttAssignedTo}</th><th>${window.translations.ganttStatus}</th><th>${window.translations.ganttPriorityMedium}</th><th>${window.translations.ganttStart}</th><th>${window.translations.ganttEnd}</th></tr>
            </thead>
            <tbody>
              ${tasksTableHtml}
            </tbody>
      ...
      <div class="two-columns">
        <div class="card">
          <div class="card-title"><span>💰</span> ${window.translations.ganttFinancialProjection}</div>
          <div style="display: flex; flex-direction: column; gap: 16px;">
            <div style="display: flex; justify-content: space-between;"><span>${window.translations.ganttEstimatedCost}</span><span style="font-weight:700;">$${costoEstimado.toLocaleString()} USD</span></div>
            <div style="display: flex; justify-content: space-between;"><span>${window.translations.ganttActualCost}</span><span style="font-weight:700; color:${variacionCosto <= 0 ? '#10b981' : '#ef4444'};">$${costoReal.toLocaleString()} USD</span></div>
            <div style="display: flex; justify-content: space-between;"><span>${window.translations.ganttVariance}</span><span style="font-weight:700; color:${variacionCosto <= 0 ? '#10b981' : '#ef4444'};">${variacionCosto <= 0 ? '+' : '-'}$${Math.abs(variacionCosto).toLocaleString()} USD</span></div>
            <div style="background:#f8fafc; padding: 12px; border-radius: 12px; margin-top: 8px;">
              <span style="font-size: 11px; color: #64748b;">${variacionCosto <= 0 ? window.translations.ganttWithinBudget : window.translations.ganttOverBudget}</span>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-title"><span>💡</span> ${window.translations.ganttStrategicRecommendation}</div>
          <div style="background: linear-gradient(135deg, ${riesgoColor}10, ${riesgoColor}05); border-radius: 16px; padding: 20px; border-left: 4px solid ${riesgoColor};">
            <p style="font-size: 14px; margin-bottom: 16px;">${window.translations.ganttRecommendationText(rezagadas, criticas, spiValue.toFixed(2))}</p>
          </div>
        </div>
      </div>
      ...
      <div class="footer">
        <p style="color: #64748b; font-size: 11px;">
          <strong>${window.translations.ganttConfidential}</strong><br>
          ${window.translations.ganttMethodology}<br>
          ${window.translations.ganttGenerated}: ${new Date().toLocaleString('en-US')} | ${window.translations.ganttSource}
        </p>
      </div>
    </div>
    ...
    <script>
      ...
      document.querySelectorAll('.gantt-row').forEach(function(row) {
        row.addEventListener('click', function() {
          ...
          alert('📋 TASK DETAIL\\n\\n📌 Task: ' + task + '\\n👤 ' + window.translations.ganttAssignedTo + ': ' + assignee + '\\n📅 ' + window.translations.ganttStart + ': ' + start + '\\n📅 ' + window.translations.ganttEnd + ': ' + end + '\\n📊 ' + window.translations.ganttStatus + ': ' + status);
        });
      });
...
console.log('✅ renderGantt traducido al inglés. Abre el modal y ve a la pestaña Gantt para probar.');
...
// ============================================================
// RENDER ASIGNACION RECURSOS - VERSIÓN INGLESA
// ============================================================
window.renderAsignacionRecursos = function(container) {
  ...
  const proyecto = window.obtenerProyectoActual ? window.obtenerProyectoActual() : null;
  if (!proyecto) {
    container.innerHTML = '<div style="text-align:center; padding:60px;">' + window.translations.resourcesNoProject + '</div>';
    return;
  }
  ...
  const recursosDisponibles = Object.values(recursos).filter(function(r) { return r.enProgreso === 0 && r.pendientes > 0; }).length;
  const recursosCriticos = Object.values(recursos).filter(function(r) { return r.atrasadas > 0; }).length;
  ...
  const html = `
    ...
    <body>
      <div class="report">
        <button class="btn-print no-print" onclick="window.print()">📄 ${window.translations.resourcesConfidential}</button>
        <div style="clear:both"></div>

        <!-- PORTADA EJECUTIVA -->
        <div class="cover">
          <span class="cover-badge">🔒 ${window.translations.resourcesConfidential}</span>
          <h1>${window.translations.resourcesTitle}</h1>
          <p>${window.translations.resourcesSubtitle}</p>
        ...
        <div class="executive-summary">
          <h2 style="margin-bottom: 16px; color: #f5a623;">📋 ${window.translations.resourcesExecutiveSummary}</h2>
          ...
          <div style="background: #f8fafc; padding: 20px; border-radius: 16px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
              <div><strong>📊 ${window.translations.resourcesKeyFindings}</strong><br>• ${tasaExito >= 80 ? window.translations.resourcesHighSuccess : tasaExito >= 60 ? window.translations.resourcesModerateSuccess : window.translations.resourcesLowSuccess}: ${tasaExito}%<br>• ${window.translations.resourcesOverloadedResources(recursosSobrecargados)}<br>• ${window.translations.resourcesOverdueTasks(tareasAtrasadas)}<br>• ${window.translations.resourcesEfficiencyPercent(eficienciaCosto)}</div>
              <div><strong>${window.translations.resourcesRecommendations}</strong><br>• ${recursosSobrecargados > 0 ? window.translations.resourcesRecommendImmediate(recursosSobrecargados) : 'Maintain current load'}<br>• ${window.translations.resourcesReviewPriorities}<br>• ${window.translations.resourcesAdjustEstimates}</div>
            </div>
          </div>
        </div>
        ...
        <!-- KPI DASHBOARD -->
        <div class="kpi-grid">
          <div class="kpi-card"><div style="color:#64748b;">${window.translations.resourcesProductivity}</div><div class="kpi-value" style="color:#10b981;">${productividadPromedio}%</div><div>${window.translations.resourcesAvgProductivity}</div></div>
          <div class="kpi-card"><div style="color:#64748b;">${window.translations.resourcesEfficiency}</div><div class="kpi-value" style="color:${eficienciaColor};">${eficienciaCosto}%</div><div>${window.translations.resourcesEfficiencyCost}</div></div>
          <div class="kpi-card"><div style="color:#64748b;">${window.translations.resourcesSuccessRate}</div><div class="kpi-value" style="color:${saludColor};">${tasaExito}%</div><div>${window.translations.resourcesOnTimeDelivery}</div></div>
          <div class="kpi-card"><div style="color:#64748b;">${window.translations.resourcesFinancialImpact}</div><div class="kpi-value" style="color:${ahorroOverrun <= 0 ? '#10b981' : '#ef4444'};">${ahorroOverrun >= 0 ? '+' : '-'}$${Math.abs(Math.round(ahorroOverrun)).toLocaleString()}</div><div>${window.translations.resourcesSavingOverrun(ahorroOverrun)}</div></div>
        </div>

        <!-- GRÁFICOS CENTRADOS -->
        <div class="chart-card">
          <h3 style="margin-bottom: 20px;">${window.translations.resourcesWorkloadDistribution}</h3>
        ...
        <div class="chart-card">
          <h3 style="margin-bottom: 20px;">${window.translations.resourcesProductivityEfficiency}</h3>
          <div style="display: flex; justify-content: center; align-items: center;">
        ...
        <div style="overflow-x: auto; margin-bottom: 32px;">
          <table class="resource-table">
            <thead><tr><th>${window.translations.resourcesResource}</th><th>${window.translations.resourcesTasks}</th><th>✅ ${window.translations.resourcesCompleted}</th><th>🔄 ${window.translations.resourcesInProgress}</th><th>⏳ ${window.translations.resourcesPending}</th><th>🔴 ${window.translations.resourcesOverdue}</th><th>${window.translations.resourcesProductivity}</th><th>${window.translations.resourcesStatus}</th></tr></thead>
            <tbody>
              ${Object.values(recursos).sort(function(a, b) { return b.enProgreso - a.enProgreso; }).map(function(r) {
                ...
                let status = '';
                let statusColor = '';
                if (r.enProgreso > 2) { status = window.translations.resourcesOverloaded; statusColor = '#ef4444'; }
                else if (r.atrasadas > 0) { status = window.translations.resourcesWithDelays; statusColor = '#f97316'; }
                else if (r.enProgreso === 0 && r.pendientes > 0) { status = window.translations.resourcesAvailable; statusColor = '#10b981'; }
                else { status = window.translations.resourcesNormal; statusColor = '#3b82f6'; }
        ...
        <div class="executive-summary" style="border-left-color: #10b981;">
          <h2 style="margin-bottom: 16px; color: #10b981;">💡 ${window.translations.resourcesRecommendations}</h2>
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px;">
            <div><strong>${window.translations.resourcesImmediateActions}</strong><br><br>• ${recursosSobrecargados > 0 ? 'Reassign tasks from ' + Object.values(recursos).filter(function(r) { return r.enProgreso > 2; }).map(function(r) { return r.nombre; }).join(', ') : 'Overloaded resources to available ones'}<br>• Review priority of ${tareasAtrasadas} overdue tasks</div>
            <div><strong>${window.translations.resourcesShortTerm}</strong><br><br>• Adjust time estimates<br>• Implement daily standups<br>• Training in workload management</div>
            <div><strong>${window.translations.resourcesLongTerm}</strong><br><br>• Review team structure<br>• Plan hires<br>• Implement OKRs per resource</div>
          </div>
        </div>

        <div class="footer">
          <p>${window.translations.resourcesConfidential}</p>
          <p>${window.translations.resourcesMethodology}</p>
          <p>${window.translations.resourcesGenerated}: ${new Date().toLocaleString('en-US')} | ${window.translations.resourcesSource}</p>
        </div>
      </div>
...
console.log('✅ renderAsignacionRecursos traducido al inglés. Abre el modal y ve a la pestaña Recursos para probar.');
...
PESTAÑA DASHBOARD
// ============================================================
// DASHBOARD (PM VIRTUAL STATS) - CORREGIDO
// ============================================================
(function() {
  if (typeof window.translations === 'undefined') window.translations = {};
  var t = window.translations;
  ...
  Object.keys(clavesDashboard).forEach(function(k) {
    if (!t[k]) t[k] = clavesDashboard[k];
  });
  ...
  window.renderPmVirtualStats = function(container) {
    ...
    if (!proyecto) {
      container.innerHTML = '<div style="color:#94a3b8;text-align:center;padding:40px;">' + t.noProject + '</div>';
      return;
    }
    ...
    var totalEstimated = tasks.reduce(function(sum, t) { return sum + (t.estimatedTime || 0); }, 0);
    var totalLogged = tasks.reduce(function(sum, t) { return sum + (t.timeLogged || 0); }, 0);
    var projectBudget = proyecto.totalProjectTime || 0;
    var budgetUsage = projectBudget > 0 ? Math.round((totalLogged / projectBudget) * 100) : 0;

    var startDate = '—', endDate = '—', totalDays = 0;
    var startDates = tasks.map(function(t) { return t.startDate; }).filter(Boolean);
    var endDates = tasks.map(function(t) { return t.deadline; }).filter(Boolean);
    if (startDates.length) {
      var min = new Date(Math.min.apply(null, startDates.map(function(d) { return new Date(d); })));
      startDate = min.toLocaleDateString('en-US');
    }
    if (endDates.length) {
      var max = new Date(Math.max.apply(null, endDates.map(function(d) { return new Date(d); })));
      endDate = max.toLocaleDateString('en-US');
      if (startDates.length) {
        totalDays = Math.ceil((max - new Date(Math.min.apply(null, startDates.map(function(d) { return new Date(d); })))) / (1000*60*60*24));
      }
    }

    var isOnTime = totalLogged <= (totalEstimated || projectBudget);
    var statusColor = isOnTime ? '#10b981' : '#ef4444';
    var statusText = isOnTime ? t.statsOnTime : t.statsBehind;

    var html = `
      <div style="padding: 20px; background: #1e1e2f; color: #e2e8f0; border-radius: 16px;">
        <h2 style="margin: 0 0 20px 0;">${t.statsTitle}</h2>
        <p><strong>${t.statsProject}:</strong> ${proyecto.name}</p>
        <hr style="border-color: #334155;">
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin: 20px 0;">
          <div style="background: #2d2d5f; padding: 15px; border-radius: 12px; text-align: center;">
            <div style="font-size: 28px; font-weight: bold;">${total}</div>
            <div>${t.statsTotalTasks}</div>
          </div>
          <div style="background: #2d2d5f; padding: 15px; border-radius: 12px; text-align: center;">
            <div style="font-size: 28px; font-weight: bold;">${completed}</div>
            <div>${t.statsCompleted}</div>
          </div>
          <div style="background: #2d2d5f; padding: 15px; border-radius: 12px; text-align: center;">
            <div style="font-size: 28px; font-weight: bold;">${pending}</div>
            <div>${t.statsPending}</div>
          </div>
          <div style="background: #2d2d5f; padding: 15px; border-radius: 12px; text-align: center;">
            <div style="font-size: 28px; font-weight: bold;">${overdue}</div>
            <div>${t.statsOverdue}</div>
          </div>
        </div>
        <div style="margin: 20px 0;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
        ...
        <div style="background: #2d2d5f; border-radius: 12px; padding: 15px; margin: 20px 0;">
          <h3 style="margin: 0 0 10px 0;">${t.statsTimeControl}</h3>
          <p><strong>${t.statsEstimated}:</strong> ${totalEstimated.toFixed(1)} h</p>
          <p><strong>${t.statsLogged}:</strong> ${totalLogged.toFixed(1)} h</p>
          <p><strong>${t.statsBudget}:</strong> ${projectBudget} h</p>
          <p><strong>${t.statsConsumption}:</strong> ${budgetUsage}%</p>
          <div style="height: 8px; background: #3b3b5f; border-radius: 4px;">
            <div style="width: ${Math.min(100, budgetUsage)}%; height: 100%; background: #8b5cf6; border-radius: 4px;"></div>
          </div>
        </div>
        <div style="display: flex; gap: 20px; background: #2d2d5f; border-radius: 12px; padding: 15px;">
          <div><strong>${t.statsStart}:</strong> ${startDate}</div>
          <div><strong>${t.statsEnd}:</strong> ${endDate}</div>
          <div><strong>${t.statsDays}:</strong> ${totalDays}</div>
        </div>
        <div style="padding: 12px; background: ${statusColor}20; border-left: 4px solid ${statusColor}; border-radius: 8px; margin-top: 20px;">
          ${t.statsStatus}: <strong style="color: ${statusColor};">${statusText}</strong>
        </div>
      </div>
    `;
  ...
  console.log('✅ Dashboard corregido. Ahora usa obtenerProyectoActual().');
})();
